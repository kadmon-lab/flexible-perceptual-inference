...
class LSTMCell(torch.nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, bias: bool = True, lesion = [], override = "LSTM", expansion = None, device=None, dtype=None) -> None:
        ...
        self.igates = torch.nn.Linear(input_dim, 4 * hidden_dim, bias=bias, **factory_kwargs).to(device)
        self.hgates = torch.nn.Linear(hidden_dim, 4 * hidden_dim, bias=bias, **factory_kwargs).to(device)
        self.gates = torch.nn.quantized.FloatFunctional().to(device)

        self.fgate_cx = torch.nn.quantized.FloatFunctional().to(device)
        self.igate_cgate = torch.nn.quantized.FloatFunctional().to(device)
        self.fgate_cx_igate_cgate = torch.nn.quantized.FloatFunctional().to(device)
        self.ogate_cy = torch.nn.quantized.FloatFunctional().to(device)
        
        self.open = torch.ones(1, hidden_dim, requires_grad = False, device = device)
        self.closed = torch.zeros(1, hidden_dim, requires_grad = False, device = device)
    ...
    def forward(self, x: Tensor, hidden: Optional[Tuple[Tensor, Tensor]] = None) -> Tuple[Tensor, Tensor]:
        ...
        x = x.squeeze(0)
        hx = hx.squeeze(0)
        hx = (1 - int('STM' in self.lesion)) * hx
        ...
        input_gate = self.open if 'INPUT' in self.lesion else input_gate
        forget_gate = self.open if 'FORGET' in self.lesion else forget_gate 
        forget_gate = self.closed if 'LTM' in self.lesion else forget_gate 
        out_gate = self.open if 'OUTPUT' in self.lesion else out_gate 

        if self.override == "RNN":
            input_gate = self.open
            out_gate = self.open
            forget_gate = self.closed 
            hy = cy = cell_gate[None, :]

        if self.override == "LSTM":
            fgate_cx = self.fgate_cx.mul(forget_gate, cx)
            igate_cgate = self.igate_cgate.mul(input_gate, cell_gate)
            cy = self.fgate_cx_igate_cgate.add(fgate_cx, igate_cgate)

            if 'STM_unbound' in self.lesion:
                tanh_cy = cy
            else:
            ...
            if 'LTM' in self.lesion:
                cy = cy*0
            
        if "LTM_bound" in self.lesion: 
            cy = torch.tanh(cy)
                        
        return hy, (hy, cy), torch.cat((forget_gate, input_gate, cell_gate, out_gate), 0)
